...
@section('content')
    <div class="p-4 center-item">
        <form class="card p-4 limited-width" method="POST" action="{{ route('register') }}">
            @csrf
            @if ($errors->any())
            ...
            <div class="form-group mb-4">
                <label for="cips">Institute ID</label>
                <input class="form-control" type="cips" name="cips" value="{{ old('cips') }}" required autofocus />
            </div>
            <div class="form-group mb-4">
            ...
            <div class="form-group mb-4">
                <label for="email">Password</label>
                <div class="position-relative">
                    <input class="form-control" type="password" name="password_confirmation" id="password_confirmation" required autofocus />
                    <div class="right-content" onclick="show_pass()">
                        <div class="eye-open d-none">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" version="1.1" id="Capa_1" width="25px" height="25px" viewBox="0 0 60.254 60.254" xml:space="preserve">
                                <g>
...
@section('extra_js')
    <script>
        function show_pass(ele) { 
            let wrapper = ele.targ.closest(".position-relative"); // Find the closest parent form-group
            let inp = wrapper.querySelector("input"); // Select the input inside that form-group
            let open_eye = wrapper.querySelector(".eye-open");
            let close_eye = wrapper.querySelector(".eye-close");
